...
if (isset($_GET['action']) && $_GET['action'] === 'getTotalData') {
    ...
    $total_data = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tb_konfirmasi"));
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover  table-striped table-sm">
                        ...
                        <tbody>
                            ...
                            // Generate the SQL query based on the search parameters
                            if (!empty($searchNoSurat) && !empty($selectedDate)) {
                                $sql = "SELECT * FROM tb_konfirmasi WHERE no_surat LIKE '%$searchNoSurat%' AND tanggal_ajuan = '$selectedDate' LIMIT $starting_limit, $data_per_page";
                            } elseif (!empty($searchNoSurat)) {
                                $sql = "SELECT * FROM tb_konfirmasi WHERE no_surat LIKE '%$searchNoSurat%' LIMIT $starting_limit, $data_per_page";
                            } elseif (!empty($selectedDate)) {
                                $sql = "SELECT * FROM tb_konfirmasi WHERE tanggal_ajuan = '$selectedDate' LIMIT $starting_limit, $data_per_page";
                            } else {
                                $sql = "SELECT * FROM tb_konfirmasi LIMIT $starting_limit, $data_per_page";
                            }
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    if ($row["tindakan"] == "Ajukan ke Kepala BPKAD") {
                                        echo '<span class="badge badge-success" style="border-radius: 50px; font-size: 16px;">Ajukan ke Kepala BPKAD</span>';
